...
export const NotificationProvider = ({ children, userId }) => {
                ...
                try {
                    ...
                    if (deviceToken && deviceToken.data) {
                        ...
                        if (token.startsWith('ExponentPushToken[')) {
                            console.warn('⚠️ WARNING: Got Expo token instead of FCM token (Store)');
                            console.warn('⚠️ This means you are likely running in Expo Go or FCM is not configured');
                            console.warn('⚠️ Push notifications will NOT work with this Edge Function');
                        } else {
                            console.log('✅ Native FCM token obtained for seller app');
                            console.log('   Token preview:', token.substring(0, 30) + '...');
                        }
                    }
                } catch (deviceTokenError) {
                    console.error('❌ Device token failed:', deviceTokenError.message);
                    console.warn('⚠️ Falling back to Expo token - notifications may not work');
                    const expoToken = await Notifications.getExpoPushTokenAsync({ projectId });
                    token = expoToken.data;
                    setExpoPushToken(token);
                    console.log('⚠️ Expo push token obtained (not compatible with FCM Edge Function)');
                    console.log('   Token preview:', token.substring(0, 30) + '...');
                }
            } catch (error) {
    ...
    useEffect(() => {
        if (!userId && fcmToken) {
            console.log('🔐 Seller logged out, unregistering FCM token');
